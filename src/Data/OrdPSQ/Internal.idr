...
public export
data Elem : (k : Type) -> (p : Type) -> (v : Type) -> Type where
  MkElem :  k
         -> p
         -> v
         -> Elem k p v

%runElab derive "Elem" [Eq,Ord,Show]
...
public export
data LTree : (k : Type) -> (p : Type) -> (v : Type) -> Type where
  Start  :  LTree k p v
  LLoser :  Size
         -> Elem k p v
         -> LTree k p v
...
%runElab derive "LTree" [Eq,Ord,Show]
...
namespace LTree
  ...
  map _ Start              =
    Start
  map f (LLoser s e l k r) =
    LLoser s (map f e) (map f l) k (map f r)
  map f (RLoser s e l k r) =
    RLoser s (map f e) (map f l) k (map f r)
...
public export
data OrdPSQ : (k : Type) -> (p : Type) -> (v : Type) -> Type where
  Void   :  OrdPSQ k p v
  Winner :  Elem k p v
         -> LTree k p v
         -> k
...
%runElab derive "OrdPSQ" [Eq,Ord,Show]
...
size' Start              =
  0
size' (LLoser s _ _ _ _) =
  s
size' (RLoser s _ _ _ _) =
  s
...
left :  LTree k p v
     -> LTree k p v
left Start               =
  assert_total $ idris_crash "Data.OrdPSQ.Internal.left: empty loser tree"
left (LLoser _ _ tl _ _) =
  tl
left (RLoser _ _ tl _ _) =
  tl
...
right :  LTree k p v
      -> LTree k p v
right Start               =
  assert_total $ idris_crash "Data.OrdPSQ.Internal.right: empty loser tree"
right (LLoser _ _ _ _ tr) =
  tr
right (RLoser _ _ _ _ tr) =
  tr
...
maxKey :  OrdPSQ k p v
       -> k
maxKey Void           =
  assert_total $ idris_crash "Data.OrdPSQ.Internal.maxKey: empty queue"
maxKey (Winner _ _ m) =
  m
...
lLoser :  k
       ...
       -> LTree k p v
       -> LTree k p v
lLoser k p v tl m tr =
  LLoser (1 + size' tl + size' tr) (MkElem k p v) tl m tr
...
rLoser :  k
       ...
       -> LTree k p v
       -> LTree k p v
rLoser k p v tl m tr =
  RLoser (1 + size' tl + size' tr) (MkElem k p v) tl m tr
