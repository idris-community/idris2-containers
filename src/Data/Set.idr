...
splitMember x (Bin _ y l r) =
  case compare x y of
    LT =>
      let (lt, found, gt) = splitMember x l
          gt'             = link y gt r
        in (lt, found, gt')
    GT =>
...
findIndex =
  ...
  where
    go :  Nat
       ...
       -> Nat
    go _   _ Tip            =
      assert_total $ idris_crash "Data.Set.findIndex: element is not in the set"
    go idx x (Bin _ kx l r) =
      case compare x kx of
        LT =>
          go idx x l
...
elemAt :  Nat
       ...
       -> a
elemAt _ Tip           =
  assert_total $ idris_crash "Data.Set.elemAt: index out of range"
...
deleteAt i t =
  case t of
    Tip         =>
      assert_total $ idris_crash "Data.Set.deleteAt: index out of range"
    Bin _ x l r =>
      case compare i (size l) of
        LT =>
...
lookupMin Tip           =
  Nothing
lookupMin (Bin _ x l _) =
  Just $ lookupMinSure x l
...
lookupMax Tip           =
  Nothing
lookupMax (Bin _ x _ r) =
  Just $ lookupMaxSure x r
...
findMin t =
  case lookupMin t of
    Just r  =>
      r
    Nothing =>
      assert_total $ idris_crash "Data.Set.findMin: empty set has no minimal element"
...
findMax t =
  case lookupMax t of
    Just r  =>
      r
    Nothing =>
      assert_total $ idris_crash "Data.Set.findMax: empty set has no maximal element"
...
deleteMin Tip              =
  Tip
deleteMin (Bin _ _ Tip r)  =
  r
deleteMin (Bin _ x l   r)  =
  balanceR x (deleteMin l) r
...
deleteMax Tip             =
  Tip
deleteMax (Bin _ _ l Tip) =
  l
deleteMax (Bin _ x l r)   =
  balanceL x l (deleteMax r)
...
deleteFindMin t =
  case minView t of
    Just res =>
      res
    Nothing  =>
      (assert_total $ idris_crash "Data.Set.deleteFindMin: can not return the minimal element of an empty set", Tip)
...
deleteFindMax t =
  case maxView t of
    Just res =>
      res
    Nothing  =>
      (assert_total $ idris_crash "Data.Set.deleteFindMax: can not return the maximal element of an empty set", Tip)
...
union t1                 Tip           =
  t1
union t1                 (Bin 1 x _ _) =
  insertR x t1
union (Bin 1 x _ _)      t2            =
  insert x t2
union Tip                t2            =
  t2
...
disjointUnion :  Set a
              ...
              -> Set (Either a b)
disjointUnion as bs =
  merge (map Left as) (map Right bs)
